...
export default async function handler(
  ...
  try {
    ...
    console.log('[DEBUG] Privy User:', JSON.stringify(privyUser, null, 2));

    // Get wallet addresses from Privy API
    const privyWalletAddresses = await getUserWalletAddresses(privyUser.id);
    console.log('[DEBUG] Privy Wallet Addresses:', privyWalletAddresses);
    ...
    if (profileError) {
      console.log('[DEBUG] Supabase profile error:', profileError);
    } else {
      console.log('[DEBUG] Supabase Profile:', JSON.stringify(supabaseProfile, null, 2));
    }
    ...
    if (allProfilesError) {
      console.log('[DEBUG] Error getting all profiles:', allProfilesError);
    } else {
      console.log('[DEBUG] Recent profiles count:', allProfiles?.length);
      allProfiles?.forEach((profile, i) => {
        console.log(`[DEBUG] Profile ${i+1}:`, {
    ...
    if (supabaseProfile?.id) {
      const { data: adminCheck, error: adminError } = await supabase
        .rpc('is_admin', { user_id: supabaseProfile.id });
      
      if (adminError) {
        console.log('[DEBUG] Admin check error:', adminError);
      } else {
        isDatabaseAdmin = adminCheck || false;
        console.log('[DEBUG] Is database admin:', isDatabaseAdmin);
      }
    }
