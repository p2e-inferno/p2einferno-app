...
export default async function handler(
  ...
  try {
    ...
    if (!adminAuthUser) {
      console.log('[ADMIN_AUTH_DEBUG] No user found in admin auth');
      return res.status(401).json({ success: false, error: "No user found via admin auth" });
    }

    console.log('[ADMIN_AUTH_DEBUG] Admin Auth User:', JSON.stringify(adminAuthUser, null, 2));
    ...
    console.log('[ADMIN_AUTH_DEBUG] Admin Wallet Addresses:', adminWalletAddresses);
    ...
    console.error("[ADMIN_AUTH_DEBUG] Error:", error);
