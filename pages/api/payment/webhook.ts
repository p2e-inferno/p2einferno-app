...
const handleSuccessfulCharge = async (supabase: any, paymentData: any) => {
  ...
  if (!extractionResult.success || !extractionResult.applicationId) {
    console.error("Webhook: Failed to extract applicationId", {
      method: extractionResult.method,
      reference: paymentReference,
      availableMetadata: paymentData.metadata ? Object.keys(paymentData.metadata) : 'none'
    });
    return;
  }
  
  const applicationId = extractionResult.applicationId;
  console.log(`Webhook: Extracted applicationId via ${extractionResult.method}:`, applicationId);
  ...
  try {
    ...
    if (queryError || !existingTransaction) {
      console.error("Webhook: No pending transaction found for applicationId:", applicationId, queryError?.message);
      return;
    }

    console.log("Webhook: Found pending transaction:", existingTransaction.id);
    ...
    if (error) {
      ...
      console.error("Payment processing failed:", data?.[0]?.message || "Unknown error");
    } else {
      console.log("Payment processed successfully:", data[0].message);
      ...
      console.log("Webhook: Updated transaction with Paystack reference:", paymentReference);
    }
  } catch (e) {
    console.error("Payment processing exception:", e);
  }
};
...
const handleFailedCharge = async (supabase: any, paymentData: any) => {
  ...
  if (!extractionResult.success || !extractionResult.applicationId || !reference) {
    console.error("Webhook: Failed charge missing required data", {
      method: extractionResult.method,
  ...
  const applicationId = extractionResult.applicationId;
  console.log(`Webhook: Extracted applicationId via ${extractionResult.method} for failed charge:`, applicationId);
  ...
  try {
    ...
    if (queryError || !existingTransaction) {
      console.error("Webhook: No pending transaction found for failed charge:", applicationId, queryError?.message);
      return;
    }
