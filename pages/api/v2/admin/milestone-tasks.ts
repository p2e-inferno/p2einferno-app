  ...
  try {
    if (req.method === 'GET') {
      ...
      if (task_id || taskId) {
        ...
        const { data, error } = await supabase.from('milestone_tasks').select('*').eq('id', id);
        if (error) return res.status(500).json({ error: 'Failed to fetch task' });
        return res.status(200).json({ success: true, data });
      }
      const mid = milestone_id || milestoneId;
      if (!mid) return res.status(400).json({ error: 'Milestone ID or Task ID is required' });
      const { data, error } = await supabase.from('milestone_tasks').select('*').eq('milestone_id', mid).order('order_index');
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supabase.from('milestone_tasks').insert(tasks).select('*');
      if (error) return res.status(500).json({ error: 'Failed to create tasks' });
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await supabase.from('milestone_tasks').update({ ...update, updated_at: new Date().toISOString() }).eq('id', id).select('*').single();
