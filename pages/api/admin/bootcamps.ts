...
const log = getLogger("api:admin:bootcamps");
...
async function createBootcamp(
  ...
  log.info("[BOOTCAMP_CREATE] Received bootcamp data:", {
    hasId: !!bootcamp?.id,
  ...
  if (bootcamp.duration_weeks === undefined || bootcamp.duration_weeks === null)
    missingFields.push("duration_weeks");
  if (bootcamp.max_reward_dgt === undefined || bootcamp.max_reward_dgt === null)
    missingFields.push("max_reward_dgt");

  if (missingFields.length > 0) {
    log.error("[BOOTCAMP_CREATE] Missing required fields:", missingFields);
  ...
  if (
    typeof bootcamp.duration_weeks !== "number" ||
    bootcamp.duration_weeks < 1
  ) {
    log.error(
      "[BOOTCAMP_CREATE] Invalid duration_weeks:",
      bootcamp.duration_weeks,
    );
    return res
      .status(400)
      .json({ error: "duration_weeks must be a positive number" });
  }

  if (
    typeof bootcamp.max_reward_dgt !== "number" ||
    bootcamp.max_reward_dgt < 0
  ) {
    log.error(
      "[BOOTCAMP_CREATE] Invalid max_reward_dgt:",
      bootcamp.max_reward_dgt,
    );
    return res
      .status(400)
      .json({ error: "max_reward_dgt must be a non-negative number" });
  }
  ...
  try {
    ...
    log.info("[BOOTCAMP_CREATE] Final bootcamp data before insert:", bootcamp);
    ...
    if (error) {
      log.error("[BOOTCAMP_CREATE] Database error:", {
        error: error,
        code: error.code,
    ...
    log.info("[BOOTCAMP_CREATE] Successfully created bootcamp:", {
      id: data[0]?.id,
      name: data[0]?.name,
    ...
    log.error("[BOOTCAMP_CREATE] Error creating bootcamp:", {
      error: error.message,
      code: error.code,
      details: error.details,
      hint: error.hint,
...
async function updateBootcamp(
  ...
  try {
    ...
    log.error("Error updating bootcamp:", error);
    return res
      .status(500)
