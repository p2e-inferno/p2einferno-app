...
const log = getLogger("api:admin:applications:reconcile");
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    log.info(`Admin reconciling application ${applicationId}`);
    log.info("Request body:", { applicationId, actions });
    ...
    log.info("Application lookup result:", { appCheck, appCheckError });
    ...
    if (!actions || actions.length === 0) {
      ...
      if (!userProfileId) {
        log.info(
          `Application ${applicationId} missing user_profile_id, trying email lookup as fallback`,
        );
        ...
        if (profileError || !userProfile) {
          log.error(
            `User profile not found for email: ${application.user_email}`,
            profileError,
          );
          return res.status(404).json({
            error:
              "User profile not found for application. User may need to create a profile first.",
          });
        }
        ...
        log.info(
          `Updated application ${applicationId} with user_profile_id ${userProfileId}`,
        );
      }
      ...
      if (!existingStatus || existingStatus.length === 0) {
        log.info(
          `Creating missing user_application_status record for application ${applicationId}`,
        );
      ...
      const reconcileResult =
        await StatusSyncService.reconcileApplicationStatus(
          applicationId,
          userProfileId,
        );
      ...
      log.info(
        `Successfully reconciled application ${applicationId} - should now be filtered out of payments list`,
      );

      return res.status(200).json({
        success: true,
        message:
          "Payment reconciled successfully! Transaction has been marked as completed.",
    ...
    log.info(
      `Admin reconciling application ${applicationId} with specific actions:`,
      actions,
    );
    ...
    if (statusError || !userAppStatus) {
      log.error("User application status not found:", statusError);
      return res.status(404).json({
    ...
    if (appDetailsError || !appDetails) {
      log.error("Application details not found:", appDetailsError);
      return res.status(404).json({
      ...
      try {
        switch (action) {
          case "approve_application":
            const approveResult = await approveApplication(
              supabase,
              application,
            );
          ...
          case "create_payment_record":
            const paymentResult = await createMissingPaymentRecord(
              supabase,
              application,
            );
          ...
          case "create_enrollment":
            const enrollmentResult = await createMissingEnrollment(
              supabase,
              application,
            );
    ...
    log.error("Admin reconciliation error:", error);
    res.status(500).json({
 ...
 */
async function createMissingPaymentRecord(
  supabase: any,
  application: any,
): Promise<ReconcileResult> {
  try {
...
async function approveApplication(
  ...
  try {
    log.info("Approving application:", application.application_id);
    ...
    if (statusError) {
      throw new Error(
        `Failed to update application status: ${statusError.message}`,
      );
    }
    ...
    if (checkError && checkError.code !== "PGRST116") {
      ...
      throw new Error(
        `Failed to check existing enrollment: ${checkError.message}`,
      );
    }
