...
const DG_NATION_LOCK = process.env
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (dbError || !withdrawalData?.success) {
      log.error("DB withdrawal initiation failed", {
        error: dbError,
        data: withdrawalData,
      });
      return NextResponse.json(
        {
          success: false,
          error: withdrawalData?.error || "Failed to initiate withdrawal",
        },
        { status: 400 },
      );
    }
    ...
    if (withdrawalData.idempotent) {
      const { data: existing } = await supabase
        ...
        .select(
          "status, transaction_hash, attestation_uid, wallet_address, amount_dg, completed_at",
        )
        .eq("id", withdrawalId)
        .single();

      const txHash = existing?.transaction_hash;
      const withdrawalTimestamp = existing?.completed_at
        ? Math.floor(new Date(existing.completed_at).getTime() / 1000)
      ...
      return NextResponse.json({
        ...
        attestationUid: existing?.attestation_uid || null,
        attestationRequired: isEASEnabled(),
        attestationPayload: isEASEnabled()
          ? {
              userAddress: existing?.wallet_address || walletAddress,
              amountDg: Number(existing?.amount_dg ?? amountDG),
              withdrawalTimestamp,
    ...
    try {
      const transferResult = await transferDGTokens(
        walletClient,
        publicClient,
        {
      ...
      if (transferResult.success && transferResult.transactionHash) {
        ...
        const withdrawalTimestamp = Math.floor(Date.now() / 1000);
        ...
        return NextResponse.json({
          ...
          attestationPayload: isEASEnabled()
            ? {
                ...
                amountDg: Number(amountDG),
                withdrawalTimestamp,
                withdrawalTxHash: transferResult.transactionHash,
              }
            : undefined,
        });
      } else {
