...
export class StatusSyncService {
  ...
  static async syncApplicationStatus({
    ...
    try {
      ...
      const currentStatus = await this.getCurrentStatus(applicationId, userProfileId);
      ...
      const newApplicationStatus = applicationStatus || current.applicationStatus;
      ...
      if (paymentStatus || applicationStatus) {
        ...
        if (applicationStatus) applicationUpdate.application_status = applicationStatus;
   ...
   */
  private static async getCurrentStatus(applicationId: string, userProfileId: string) {
    try {
      ...
      if (userError) throw new Error(`Failed to get user status: ${userError.message}`);
      ...
      return {
        ...
        data: {
          ...
          applicationStatus: applicationData.application_status as ApplicationStatus,
          enrollmentStatus: enrollmentData?.enrollment_status as EnrollmentStatus | undefined,
  ...
  private static validateStatusTransitions(
    ...
    if (current.paymentStatus !== proposed.paymentStatus) {
      if (!canTransition('payment', current.paymentStatus, proposed.paymentStatus)) {
        return {
          valid: false,
    ...
    if (current.applicationStatus !== proposed.applicationStatus) {
      if (!canTransition('application', current.applicationStatus, proposed.applicationStatus)) {
        return {
          valid: false,
    ...
    if (proposed.enrollmentStatus && current.enrollmentStatus !== proposed.enrollmentStatus) {
      if (!canTransition('enrollment', current.enrollmentStatus, proposed.enrollmentStatus)) {
        return {
          valid: false,
   ...
   */
  static async reconcileApplicationStatus(applicationId: string, userProfileId: string) {
    try {
      const currentStatus = await this.getCurrentStatus(applicationId, userProfileId);
      if (!currentStatus.success) {
        return { success: false, error: currentStatus.error };
