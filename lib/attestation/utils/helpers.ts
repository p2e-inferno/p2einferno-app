 ...
 */
export const getTimeUntilExpiration = (expirationTime?: string): string | null => {
  if (!expirationTime) {
    return null;
  }
 ...
 */
export const formatAttestationDataForDisplay = (data: Record<string, any>): Record<string, string> => {
 ...
 */
export const truncateAttestationUid = (uid: string, startChars = 6, endChars = 4): string => {
 ...
 */
export const parseSchemaDefinition = (definition: string): Array<{ type: string; name: string }> => {
  if (!definition.trim()) {
    return [];
  }
