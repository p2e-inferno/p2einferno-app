 ...
 */
export const getAttestationByUid = async (attestationUid: string): Promise<Attestation | null> => {
  try {
 ...
 */
export const getUserDailyCheckinStreak = async (userAddress: string): Promise<number> => {
  try {
 ...
 */
export const getSchemaStatistics = async (schemaUid: string): Promise<{
  totalCount: number;
  uniqueUsers: number;
  todayCount: number;
  ...
  try {
    ...
    const uniqueUsers = new Set(uniqueUsersData?.map(a => a.recipient) || []).size;
