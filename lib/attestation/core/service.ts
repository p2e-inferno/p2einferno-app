...
export class AttestationService {
   ...
   */
  async createAttestation(params: CreateAttestationParams): Promise<AttestationResult> {
    try {
      ...
      const encodedData = this.encodeDataForSchema(schema.schema_definition, data);
      ...
      const existingAttestation = await this.checkExistingAttestation(recipient, schemaUid);
   ...
   */
  async revokeAttestation(params: RevokeAttestationParams): Promise<AttestationResult> {
    try {
      ...
      const easContract = new ethers.Contract(EAS_CONFIG.CONTRACT_ADDRESS, EAS_ABI, signer);
   ...
   * Get schema definition from database
   */
  private async getSchemaDefinition(schemaUid: string): Promise<AttestationSchema | null> {
    try {
