...
export const validateAuthConfiguration = (): AuthConfigValidation => {
  ...
  const adminRequiredVars = ["NEXT_PUBLIC_ADMIN_LOCK_ADDRESS"];
  ...
  if (context.type === "server") {
    ...
    const missingRequired = requiredServerEnvVars.filter(
      (key) => !process.env[key],
    );
    ...
    const missingAdminRequired = adminRequiredVars.filter(
      (key) => !process.env[key],
    );
    const missingAdminOptional = adminOptionalVars.filter(
      (key) => !process.env[key],
    );
...
export const checkAuthFeatureAvailability = () => {
  return {
    privyAuth: !!(
      process.env.NEXT_PUBLIC_PRIVY_APP_ID && process.env.NEXT_PRIVY_APP_SECRET
    ),
