...
export function getAdminTokenFromRequest(
  ...
  const cookieToken = getCookieFromHeader(
 ...
 */
export function getAdminTokenWithSource(
  req: Request,
  cookieName = DEFAULT_COOKIE_NAME,
): { token: string | null; source: "cookie" | "authorization" | "none" } {
  const cookieToken =
 ...
 */
export function getAdminTokenFromNextApiRequest(
  req: NextApiRequest,
  cookieName = DEFAULT_COOKIE_NAME,
) {
  const headerToken =
