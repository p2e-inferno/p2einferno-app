...
// Intentionally resolved at module load: DG Vendor is a fixed deployed contract
// address and is not expected to change at runtime for this app instance.
...
export class VendorVerificationStrategy implements VerificationStrategy {
  ...
  async verify(
    ...
    const vendorAddress = this.resolveVendorAddress();
    if (!vendorAddress) {
      return {
        success: false,
  ...
  private async verifyTransaction(
    ...
    if (receipt.to?.toLowerCase() !== vendorAddress.toLowerCase()) {
      return {
        success: false,
    ...
    if (!decoded) {
      return {
        success: false,
        error: "Expected vendor event not found",
  ...
  private getExpectedEvent(
    type: TaskType,
  ): "TokensPurchased" | "TokensSold" | "Lit" | null {
    switch (type) {
      case "vendor_buy":
  ...
  private resolveVendorAddress(): Address | null {
    if (!VENDOR_ADDRESS || !isAddress(VENDOR_ADDRESS)) {
      return null;
    }
    return VENDOR_ADDRESS as Address;
  }
}
