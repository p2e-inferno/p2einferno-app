import { issueAdminSession, verifyAdminSession } from '@/lib/auth/admin-session';

describe('admin-session', () => {
  test('issues and verifies a token with correct claims (mocked jose)', async () => {
    const { token, exp } = await issueAdminSession({ did: 'did:privy:abc', wallet: '0xabc', roles: ['admin'], locks: ['0xlock'] }, 10);
