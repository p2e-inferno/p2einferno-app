...
describe("admin session route", () => {
  beforeEach(() => {
    ...
    (privy.getPrivyUser as any).mockResolvedValue({
      id: "did:privy:xyz",
      walletAddresses: ["0xabc"],
    });
    // Route uses getPrivyUserFromNextRequest; mock it to return the same shape
    (privy.getPrivyUserFromNextRequest as any).mockResolvedValue({
      id: "did:privy:xyz",
      walletAddresses: ["0xabc"],
    });
    (keychecker.checkMultipleWalletsForAdminKey as any).mockResolvedValue({
      hasValidKey: true,
    });
    // Wallet addresses lookup used by route
    (privy.getUserWalletAddresses as any).mockResolvedValue(["0xabc"]);
  });
  ...
  test("returns 401 if not authenticated", async () => {
    ...
    (privy.getPrivyUserFromNextRequest as any).mockResolvedValue(null);
