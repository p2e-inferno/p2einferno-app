...
import { resolvePoolTokens, validateFeeConfig, FEE_CONFIG, UNISWAP_ADDRESSES } from "@/lib/uniswap/constants";

describe("resolvePoolTokens", () => {
  const weth = UNISWAP_ADDRESSES.weth;

  it("identifies WETH as token0", () => {
    const result = resolvePoolTokens(weth, "0xaC27fa800955849d6D17cC8952Ba9dD6EAA66187");
    expect(result.wethToken).toBe(weth);
    expect(result.otherToken).toBe("0xaC27fa800955849d6D17cC8952Ba9dD6EAA66187");
  });

  it("identifies WETH as token1", () => {
    const result = resolvePoolTokens("0xaC27fa800955849d6D17cC8952Ba9dD6EAA66187", weth);
    expect(result.wethToken).toBe(weth);
    expect(result.otherToken).toBe("0xaC27fa800955849d6D17cC8952Ba9dD6EAA66187");
  });

  it("is case-insensitive", () => {
    const upperWeth = UNISWAP_ADDRESSES.weth.toUpperCase().replace("0X", "0x") as `0x${string}`;
    const result = resolvePoolTokens(upperWeth, "0xaC27fa800955849d6D17cC8952Ba9dD6EAA66187");
...
describe("validateFeeConfig", () => {
  ...
  it("throws when fee wallet is not set", () => {
    ...
    expect(() => validateFeeConfig()).toThrow("NEXT_PUBLIC_UNISWAP_FEE_WALLET is not configured");
  ...
  it("passes when fee wallet is a valid address", () => {
    const savedRecipient = FEE_CONFIG.feeRecipient;
    (FEE_CONFIG as any).feeRecipient = "0x1234567890abcdef1234567890abcdef12345678";
