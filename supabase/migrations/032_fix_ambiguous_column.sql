-- Fix ambiguous column reference in handle_successful_payment function
...
BEGIN
    ...
    -- 5. Create or update the user_application_status to 'enrolled' (fixed ambiguous column)
    ...
    VALUES (
        ...
        CASE 
            ...
            ELSE p_payment_method
        END,
        -- Get amount and currency from the application (use subquery to avoid ambiguity)
        (SELECT total_amount FROM public.applications WHERE id = p_application_id),
        (SELECT currency FROM public.applications WHERE id = p_application_id),
        NOW()
    )
